...
void setup() {
    ...
    allNodes[0].neighbors.push_back({1, 40.0f}); // từ node 0 đến node 1 là 40 
...
bool runVoiceInferenceCycleABC() {
    // Start recording
    // bắt đầu record thì nó sẽ luôn vàng trong quá trình record
    digitalWrite(LED_RECORD, HIGH);
    bool m = microphone_inference_record();
    digitalWrite(LED_RECORD, LOW);

    // có bất kỳ lỗi gì sẽ đều nhấp nháy 3 lần
    if (!m) {
    ...
    if (result.classification[1].value > result.classification[0].value && 
        ...
        // nếu đúng thì đèn xanh nhấp nháy 3 lần
        for (int i = 0; i < 3; i++) {
...
bool runVoiceInferenceCycleD() {
    ...
    // nếu m sai thì đèn đỏ nhấp nháy 3 lần
    if (!m) {
        ei_printf("ERR: Failed to record audio...\n");
        digitalWrite(LED_ERROR, HIGH);
        delay(500);
...
void callback(char* topic, uint8_t* payload, unsigned int length) {
    ...
    if (!unlocked) {
        ...
        if (authSuccess) {
            ...
            // không cần bật đèn xanh ở đây nữa vì đã tích hợp trong phần nhận diện giọng nói rồi
            delay(5000);
